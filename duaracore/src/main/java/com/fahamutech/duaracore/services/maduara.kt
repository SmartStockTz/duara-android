...
suspend fun syncContacts(context: Context): MutableList<DuaraRemote> {
    ...
    return if (user?.pub != null) {
        val maduaraSignatures: Array<String> =
            context.resources.getStringArray(R.array.maduara_signatures)
        val contacts =
            if (maduaraSignatures.isEmpty()) arrayOf(getDeviceId(context.contentResolver))
        ...
        syncSendModel.meta.age = user.age
        syncSendModel.meta.gender = user.gender
